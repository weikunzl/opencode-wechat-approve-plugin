...
export class ApprovalManager {
  ...
  async onPermissionReplied(event: PermissionRepliedEvent): Promise<void> {
    ...
    const remaining = current.filter((item) => item.requestID !== event.properties.requestID)
    this.store.savePendingApprovals(remaining)
    const conversation = this.store.loadConversation()
    if (!conversation?.requestIDs.includes(event.properties.requestID)) return
    // A reply generated by this manager can arrive as an OpenCode event while
    // the partial-selection follow-up is being persisted. Keep its original
    // ordinal snapshot until every request is resolved.
    if (conversation.selectionOnly && remaining.length > 0) return
