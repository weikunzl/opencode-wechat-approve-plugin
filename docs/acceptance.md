...
1.1.3 的重启健康恢复变更按
[`docs/release-impact-1.1.3.md`](release-impact-1.1.3.md) 执行增量门禁。
`doctor` 中 binding 仅证明凭据存在；transport 为 healthy 且具有最近成功时间，
才证明本次运行的真实发送探测成功。全部 OpenCode 实例退出后不存在插件外监控。
