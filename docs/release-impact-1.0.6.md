...
使用 registry `1.0.6` 执行安装后重启 `opencode web` 或新建会话，先确认 `permission.asked` 与 pending=1。随后按 [`approval-security-matrix.md`](approval-security-matrix.md) 重跑 REAL-02～REAL-14；每项只以完整人工文字证据或严格屏幕证据判定通过。REAL-00 与 REAL-01 的 1.0.5 历史证据不能替代本版本的审批回归。
