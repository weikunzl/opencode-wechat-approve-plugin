...
状态扫描使用 `npm run test:e2e:status` 做一轮无副作用检查；需要持续观察时显式运行
`npm run test:e2e:status -- --interval=30000`。扫描器只报告 OpenCode pending、本地
pending/outbox 和 context 年龄，微信标题、原文及 requestID/decision 仍必须人工核对。
...
REAL-00 至 REAL-18 的逐项前置条件、操作、预期和证据规则见 [`docs/approval-security-matrix.md`](approval-security-matrix.md)。
