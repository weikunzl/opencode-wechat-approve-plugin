...
严格模式要求原始标题恰为 `微信ClawBot`。用户明确选择人工模式时，标题不匹配只能记录为 `MANUAL_REPORTED` 或 `MANUAL_CONFIRMED`，不能改写为严格标题 `PASS`。

证据分层：严格标题 `PASS` 仍代表屏幕证据；人工模式的 `MANUAL_REPORTED` 表示完整文字证据，`MANUAL_CONFIRMED` 表示用户身份/目标确认。人工模式不要求截图，但必须记录原始标题、用户确认、可见原文和操作者时间。
...
| `status` | `PASS`、`MANUAL_REPORTED`、`MANUAL_CONFIRMED`、`BLOCKED` 或 `UNVERIFIED` |

示例（仅为格式，不是真实证据）：

```json
{"scenarioID":"REAL-00","packageVersion":"1.0.5","originalWindowTitle":"微信 ClawBot","manualConfirmation":"MANUAL_CONFIRMED","userConfirmation":"<用户明确确认>","wechatTextSummary":"[Done] REAL-00 retry-00 registry smoke ...","pluginTextSummary":"[Done] REAL-00 retry-00 registry smoke ...","requestDecisions":"无审批请求","pendingBefore":0,"pendingAfter":0,"outboxBefore":0,"outboxAfter":0,"conversationRecord":"<脱敏对话记录>","cleanupResult":"无 pending/outbox（只读快照）","operatorTime":"2026-07-28T05:14:33.899Z","status":"MANUAL_REPORTED"}
```

## 主线程校验与安全规则

- 缺少任一字段、版本不符、标题未记录、用户未确认、对话记录不可定位或 pending 清理失败时，状态只能是 `UNVERIFIED` 或 `BLOCKED`。
- `MANUAL_REPORTED` 需要完整文字证据；`MANUAL_CONFIRMED` 只能证明用户确认，不能替代完整场景记录。
- 严格标题 `PASS` 与人工文字证据必须分开记录，不得把文字证据伪装成屏幕 PASS。
