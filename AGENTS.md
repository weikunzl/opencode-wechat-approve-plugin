...
- REAL-00 必须先通过：窗口标题严格为 `微信ClawBot`，并观察到脱敏的微信原文。每个标记为 `PASS` 的场景都必须有脱敏微信截图索引。
- 任一真实线程场景为 `BLOCKED` 或 `UNVERIFIED` 时，不得将其标记为通过，也不得以该状态发布；自动化、HTTP、内存或 fake 证据不能替代真实截图。
...
- REAL-00 必须由主流程先创建无副作用通知，用户确认屏幕原文并提供截图后，真实线程才能按顺序推进 REAL-01～REAL-18；主线程须校验结构化日志的版本、标题、确认、原文摘要、脱敏决策、pending/outbox、截图索引和清理结果。
